...
export default function Index() {
    ...
    const handleViewItem = async (
        ...
        if (activity_type === "contents") {
            await api
                .get(`/api/content/${activity_id}`)
                .then((res) => {
                    if (res.status === 200) {
                        router.push(
                            `/dashboard/contents/view-details/${activity_id}`
                        );
                    }
                })
                .catch((err) => {
        ...
        if (activity_type === "collections") {
            await api
                .get(`/api/collection/${activity_id}`)
                .then((res) => {
                    if (res.status === 200) {
                        router.push(
                            `/dashboard/collections/view-contents/${activity_id}`
                        );
                    }
                })
                .catch((err) => {
