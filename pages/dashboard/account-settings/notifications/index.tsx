...
const Notifications = () => {
    ...
    const { isLoading, data, refetch } = useQuery(
        ...
        () => api.get(`/api/weekly-digests-notification/${team.id}`),
        {
            enabled: !!team.id,
            onSuccess: (data) => {
                    ...
                    <div>
                        {notifications.map((notification) => {
                            return (
                                <div
                                    key={notification.id}
                                    className="xs:border-b xs:border-[#0000001a] sm:border-none xs:last:border-none w-full sm:bg-[#FFFFFF] md:rounded-[4px] md:h-[159px] xl:h-[137px] 2xl:h-[115px] sm:p-[30px] md:mb-[16px] xs:bg-inherit flex justify-between items-center relative"
                                >
