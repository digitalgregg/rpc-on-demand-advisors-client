...
function WishList() {
  ...
  const { isLoading, data, refetch } = useQuery("get wish", () => api.get(`/api/wish?team_id=${team.id}`));
        ...
        {wishes?.data.map((wish: any, index: any) => (
          <div key={index}>
            ...
            <div className="p-[15px]">
              ...
              <button onClick={() => router.push(`/dashboard/wishlist/view/${wish._id}`) } className="text-xs leading-[40px] text-primary hover:text-[#fff] font-semibold w-full border-primary border transition-all duration-200 hover:bg-primary rounded-[4px] h-[40px]">
                Manage
              </button>
            </div>
          </div>
